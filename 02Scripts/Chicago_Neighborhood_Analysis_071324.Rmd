...
Seg_building$NumConst_since15[match(rownames(perm_yrs_const), Seg_building$Segment)]  <- rowSums(perm_yrs_const[,c("2015", "2016", "2017")])
...
Seg_allData$Change_PRent <- Seg_allData$PercRent21 - Seg_allData$PercRent11
Seg_allData$Change_Rmov <- Seg_allData$PercRMov21 - Seg_allData$PercRMov11
...
ChiCors <- cor(Seg_allData[,c("PercComm_z", "PercRes_z","NumConst_since15","NumReno_since15", 
                   "NumBuild", "SVI", "AftTemp","Change_PHL", "Change_PBl","Change_PWnHL",
...
##maybe do percent res instead and then can have Afternoon Temp in all of them too
##either num construction or num reno, either chan go with Change PWnHL, and Change PRent (too correlated to Change MedHHI and rmov)
##SVI can go with the Change in WnHL, Change MedHHI, Change PRent, and Change Rmov
##can do all the changes with Percent rentership or Median Household income but can't put those two together

##Because of the correlations above, I think it makes sense to simplify
#1 - Percent commercial tree + Afternoon Temp + NumBuild
#2 - Percent commercial tree + SVI + ChangePWhite + Change MedHHI + Change PRent + Change Rmov
#3 - Percent commercial tree + NumConstruction + ChangePWhite + Change MedHHI + Change PRent + Change Rmov
#4 - Percent commercial tree + NumReno + ChangePWhite + Change MedHHI + Change PRent + Change Rmov

f1 <- "PercCOMM_MxTree + AftTemp + NumBuild"
...
##is it worth re-scaling and centering the data in order to make the models tractable?

#############################Sidewalk cutout
mods_m1 <- paste(rep("PercSC ~", 5), c(f1, f2, f3, f4))
...
AIC(bestf1mod.1, bestf1mod.2, bestf1mod.3, bestf1mod.4)

#             df       AIC
# bestf1mod.1  5 -2266.112
# bestf1mod.2  8 -3348.053
# bestf1mod.3  8 -3433.535**
# bestf1mod.4  8 -3303.322


#write.csv(f1sum.3$coefficients$mean, "../04Outputs/Chicago/Chi_SC_Best_m3.csv")

###################################Afternoon Temperature - linreg
mods_m2 <- paste(rep("AftTemp ~", 5), c(f1, f2, f3, f4))
...
AIC(bestf2mod.2, bestf2mod.3, bestf2mod.4)

#             df       AIC
# bestf2mod.1  2 10.687082
# bestf2mod.2  4  4.852855*
# bestf2mod.3  4  4.852855
# bestf2mod.4  4  4.852855


#write.csv(f2sum.2$coefficients, "../04Outputs/Chicago/Chi_Temperature_Best_m2.csv")

###########################Percent Gardenscape 3 or 4
mods_m3 <- paste(rep("PercGar3o4 ~", 5), c(f1, f2, f3, f4))
...
AIC(bestf3mod.1, bestf3mod.2, bestf3mod.3, bestf3mod.4)

#             df       AIC
# bestf3mod.1  5 -2101.254**
# bestf3mod.2  8 -1873.089
# bestf3mod.3  7 -1933.455
# bestf3mod.4  8 -1905.296



#write.csv(f3sum.1$coefficients$mean, "../04Outputs/Chicago/Chi_Gardenscape_Best_m1.csv")
...
mods_m4 <- paste(rep("SimpsonsDivSpp ~", 5), c(f1, f2, f3, f4))
...
AIC(bestf4mod.1, bestf4mod.2, bestf4mod.3, bestf4mod.4)

#             df       AIC
# bestf4mod.1  5 -3092.479
# bestf4mod.2  6 -3246.986
# bestf4mod.3  7 -3371.848**
# bestf4mod.4  6 -3303.384



#write.csv(f4sum.3$coefficients$mean, "../04Outputs/Chicago/Chi_SimpsonSpp_Best_m3.csv")

############################ Simpson Diversity - genus
mods_m5 <- paste(rep("SimpsonsDivGen ~", 5), c(f1, f2, f3, f4))
...
AIC(bestf5mod.1, bestf5mod.2, bestf5mod.3, bestf5mod.4)

#             df       AIC
# bestf5mod.1  4 -2401.292
# bestf5mod.2  7 -2616.589
# bestf5mod.3  8 -2701.798 **
# bestf5mod.4  7 -2626.989

#write.csv(f5sum.3$coefficients$mean, "../04Outputs/Chicago/Chi_SimpsonGen_Best_m3.csv")


###AvgBA
mods_m6 <- paste(rep("AvgBA ~", 5), c(f1, f2, f3, f4))
...
AIC(bestf6mod.1, bestf6mod.2, bestf6mod.3, bestf6mod.4)

#             df       AIC
# bestf6mod.1  3 -43.16004 *
# bestf6mod.2  6 -35.79276
# bestf6mod.3  6 -35.79276
# bestf6mod.4  6 -35.79276


#write.csv(f6sum.1$coefficients, "../04Outputs/Chicago/Chi_AvgBA_Best_m1.csv")

#This model should be different because should not use number of commercial
##trees as a predictor here
f1.2 <- "PercComm_z + AftTemp + NumBuild"
f2.2 <- "PercComm_z + SVI + Change_PWnHL + Change_MedHHI + Change_PRent + Change_Rmov"
f3.2 <- "PercComm_z + NumConst_since15 + Change_PWnHL + Change_MedHHI + Change_PRent + Change_Rmov"
f4.2 <- "PercComm_z + NumReno_since15 + Change_PWnHL + Change_MedHHI + Change_PRent + Change_Rmov"

##Number of Trees
mods_m7 <- paste(rep("totNumTrees ~", 5), c(f1.2, f2.2, f3.2, f4.2))
...
AIC(f7mod.1, f7mod.2, f7mod.3, f7mod.4)
...
#write.csv(f7sum.1$coefficients, "../04Outputs/Chicago/Chi_NumTrees_Best_m1.csv")

###Urban Tolerance
mods_m8 <- paste(rep("PercUrbanApp ~", 5), c(f1, f2, f3, f4))
...
AIC(bestf8mod.1, bestf8mod.2, bestf8mod.3, bestf8mod.4)

#             df       AIC
# bestf8mod.1  5 -1290.162
# bestf8mod.2  7 -1589.820 *
# bestf8mod.3  7 -1589.820
# bestf8mod.4  7 -1589.820


#write.csv(f8sum.2$coefficients, "../04Outputs/Chicago/Chi_UrbanApp_Best_m2.csv")
##########################################################Tree Health

###########################PercDisc3o4
mods_m9 <- paste(rep("PercDisc3o4 ~", 5), c(f1, f2, f3, f4))
...
AIC(bestf9mod.1, bestf9mod.2, bestf9mod.3, bestf9mod.4)

#             df       AIC
# bestf9mod.1  5 -2853.530
# bestf9mod.2  8 -3583.963
# bestf9mod.3  8 -3704.853 *
# bestf9mod.4  8 -3646.985

#write.csv(f9sum.3$coefficients, "../04Outputs/Chicago/Chi_Discol_Best_m3.csv")

###########################PercDBK5
mods_m10 <- paste(rep("PercDBK5 ~", 5), c(f1, f2, f3, f4))
...
AIC(bestf10mod.1, bestf10mod.2, bestf10mod.3, bestf10mod.4)

#              df       AIC
# bestf10mod.1  4 -2923.441
# bestf10mod.2  7 -3126.811
# bestf10mod.3  8 -3424.860 *
# bestf10mod.4  7 -3159.317

#write.csv(f10sum.3$coefficients, "../04Outputs/Chicago/Chi_DBK_Best_m3.csv")

###########################PercCS4o5
mods_m11 <- paste(rep("PercCS4o5 ~", 5), c(f1, f2, f3, f4))
...
AIC(bestf11mod.1, bestf11mod.2, bestf11mod.3, bestf11mod.4)

#              df       AIC
# bestf11mod.1  5 -2540.697
# bestf11mod.2  8 -3314.847
# bestf11mod.3  8 -3605.916 *
# bestf11mod.4  8 -3318.19


#write.csv(f11sum.3$coefficients, "../04Outputs/Chicago/Chi_CS_Best_m3.csv")
...
modList <- list(f1sum.3, f2sum.2, f3sum.1, f4sum.3, f5sum.3, f6sum.1, f7sum.1, f8sum.2, f9sum.3, f10sum.3, f11sum.3)
...
write.csv(ModOverview, "../04Outputs/Chicago/Chi_ModelOverviewResults_Since15_071324.csv")
